...
public class AssetOpeningDao extends BaseDao {
    ...
    public List<DropdownDTO> getFixedAssetGroupList() {
        ...
        return session.createSQLQuery(query).setParameter("groupTypeId", AccountTypeEnum.NON_CURRENT_ASSET.getValue()).setResultTransformer(Transformers.aliasToBean(DropdownDTO.class)).list();
    }
    ...
    public void save(FaPurchase fa_purchase) {
        sessionFactory.getCurrentSession().saveOrUpdate(fa_purchase);
    }
    ...
    public List<OpeningAndBuyingDTO> getItemSuggestionList(Integer companyId) {
        ...
        Session session = sessionFactory.getCurrentSession();
        return session.createSQLQuery(query).setParameter("companyId", companyId).setResultTransformer(Transformers.aliasToBean(OpeningAndBuyingDTO.class)).list();
    }
    ...
    public void saveToMasterTable(FaPurchaseMaster faPurchaseMaster) {
        sessionFactory.getCurrentSession().saveOrUpdate(faPurchaseMaster);
    }
    ...
    public void savePurchaseDetail(FaPurchaseDetail faPurchaseDetail) {
        sessionFactory.getCurrentSession().saveOrUpdate(faPurchaseDetail);
    }
    ...
    public String getMaxAssetCodeSerialNo(Integer companyId) {
        ...
        Session session = sessionFactory.getCurrentSession();
        return (String) session.createSQLQuery(query).setParameter("companyId", companyId).uniqueResult();
    }
    ...
    public List<OpeningAndBuyingDTO> getInvoiceDetailList(String purchaseInvoiceNo, Integer companyId) {
        ...
        Session session = sessionFactory.getCurrentSession();
        return session.createSQLQuery(query).setParameter("companyId", companyId).setParameter("purchaseInvoiceNo", purchaseInvoiceNo).setResultTransformer(Transformers.aliasToBean(OpeningAndBuyingDTO.class)).list();
    }
...
*/
        ...
        return session.createSQLQuery(sqlQuery).setParameter("companyId", companyId).setResultTransformer(Transformers.aliasToBean(OpeningAndBuyingDTO.class)).list();
    }

    @Transactional(readOnly = true)
    public List<OpeningAndBuyingDTO> loadAssetOpeningList(BigInteger faPurchaseId) {
        String sqlQuery = "SELECT b.assetId,b.assetDetailId, " + "  a.faPurchaseId," + "  d.purchaseMasterId,\n" + "  b.particular,\n" + "  a.purchaseDate,\n" + "  a.openingBalance as amount, \n" + "  a.depreciatedValue,\n" + "  a.rate,\n" + "  a.qty,\n" + "  e.accTypeId \n" + " FROM tbl_fa_purchase a\n" + "INNER JOIN tbl_fa_item_setup_detail b ON a.assetDetailId=b.assetDetailId\n" + "INNER JOIN tbl_fa_item_setup c ON c.assetId=b.assetId\n" + "INNER JOIN tbl_fa_purchase_mater d ON d.purchaseMasterId=a.purchaseMasterId\n" + "INNER JOIN tbl_fa_group e ON c.assetClassId=e.assetClassId\n" + "WHERE a.faPurchaseId=:faPurchaseId";

        return sessionFactory.getCurrentSession().createSQLQuery(sqlQuery).setParameter("faPurchaseId", faPurchaseId).setResultTransformer(Transformers.aliasToBean(OpeningAndBuyingDTO.class)).list();
    }
    ...
    public void deleteItemFromDetail(BigInteger faPurchaseId) {
        ...
        sessionFactory.getCurrentSession().createSQLQuery(sqlQuery).setParameter("faPurchaseId", faPurchaseId).executeUpdate();
    }
    ...
    public boolean isOpeningAssetCount(BigInteger purchaseMasterId) {
        ...
        return sessionFactory.getCurrentSession().createSQLQuery(sqlQuery).setParameter("purchaseMasterId", purchaseMasterId).uniqueResult().equals(BigInteger.ZERO);
    }
    ...
    public void deleteItemFromMaster(BigInteger purchaseMasterId) {
        ...
        sessionFactory.getCurrentSession().createSQLQuery(sqlQuery).setParameter("purchaseMasterId", purchaseMasterId).executeUpdate();
    }
    ...
    @Transactional(readOnly = true)
    public Integer getAccTypeByAssetItem(BigInteger assetDetailId) {
