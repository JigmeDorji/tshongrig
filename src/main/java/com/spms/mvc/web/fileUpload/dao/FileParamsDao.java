...
@Repository
public class FileParamsDao {
    @Autowired
    SessionFactory sessionFactory;

    @Transactional
    public Integer getLastCountVal(Integer companyId) {
        String sqlQry = "select count(*) from tbl_file_params  where companyId=:companyId";
        SQLQuery hibernate = sessionFactory.getCurrentSession().createSQLQuery(sqlQry);
        BigInteger countBigInteger = (BigInteger) hibernate
                .setParameter("companyId", companyId)
                .uniqueResult();
        String countStringVal = String.valueOf(countBigInteger);
        return Integer.valueOf(countStringVal);
    }

    @Transactional
    public void saveFileParams(FileParams fileParams) {
        sessionFactory.getCurrentSession().saveOrUpdate(fileParams);
    }
