...
assetOpening = (function () {
                ...
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.status === 1) {
                                swal({
                                    timer: 800, type: "success", title: res.text, showConfirmButton: false
                                }, function () {
                                ...
                                swal({
                                    title: res.text, text: "Click OK to exit", type: "warning"
                                }, function () {
                                });
                            }
    ...
    function populateData() {
        $.ajax({
            url: 'receivedItem/getRecentPurchaseData', type: 'GET', success: function (res) {
                if (res !== '') {
    ...
    function loadDropdown() {
        ...
        $('#autoCompleteId' + iterator).typeahead({
            ...
            name: 'items', displayKey: 'value', source: items.ttAdapter(), templates: {
                notFound: '<div>Not Found</div>',
                pending: '<div>Loading...</div>', // header: '<div>Found Records:</div>',
    ...
    function calculateTotal() {
        ...
        $('#fixedAssetOpeningGrid tbody').find('tr').each(function () {
            ...
            if (typeof row.find('.rate').val() != 'undefined' && typeof row.find('.qty').val() != 'undefined') {
                row.find('.total').val((row.find('.rate').val() === '' ? 0 : parseFloat(row.find('.rate').val())) * (row.find('.qty').val() === '' ? 0 : parseFloat(row.find('.qty').val())));
                grandTotalAmount = grandTotalAmount + (row.find('.rate').val() === '' ? 1 : parseFloat(row.find('.rate').val())) * (row.find('.qty').val() === '' ? 1 : parseFloat(row.find('.qty').val()));
            }
        });
    ...
    function _disPlayData() {
        ...
        spms.ajax(baseURL() + 'loadAssetOpeningList', 'GET', {
            ...
            for (let i = 0; i < res.length; i++) {
                ...
                let row = "<tr>" + "<td><input type='text' readonly class='form-control rowNumber'>" + "<input type='hidden' id='assetDetailId" + iterator + "' name='openingAndBuyingListDTO[0].assetDetailId'  class='form-control' value='" + res[i].assetDetailId + "'>" + "<input type='hidden' id='fixedAssetGroupId' name='openingAndBuyingListDTO[0].fixedAssetGroupId'  class='form-control'>" + "<input type='hidden' id='faPurchaseId' name='openingAndBuyingListDTO[0].faPurchaseId'  class='form-control faPurchaseId' value='" + res[i].faPurchaseId + "'></td>" + "<td><input type='text' name='openingAndBuyingListDTO[0].particular' value='" + res[i].particular + "' class='form-control particular'></td>" + "<td><input type='text'  name='openingAndBuyingListDTO[0].purchaseDate' class='form-control fxFormatDate' value='" + formatAsDate(res[i].purchaseDate) + "'></td>" + "<td><input type='text'  name='openingAndBuyingListDTO[0].openingBalance' class='form-control openingBalance right-align' value='" + res[i].amount + "'></td>" + "<td><input type='text'  name='openingAndBuyingListDTO[0].depreciatedValue' class='form-control depreciatedValue right-align'value='" + res[i].depreciatedValue + "'></td>" + "<td><input type='text'  name='openingAndBuyingListDTO[0].rate' class='form-control rate right-align' value='" + res[i].rate + "'></td>" + "<td><input type='text'  name='openingAndBuyingListDTO[0].qty' class='form-control qty  right-align' value='" + res[i].qty + "'></td>" + "<td><input type='text'  name='openingAndBuyingListDTO[0].total' readonly class='form-control total right-align' value='" + totalAmount + "'></td>" + "<td class='text-center'><button class='btn btn-danger btn-xm  d-sm-inline-block removeBtn' type='button' id='removeBtn'><i class='fa fa-trash'></i> Delete</button>" + "<button class='btn btn-xm btn-success d-sm-inline-block addBtn' type='button' id='addBtn'><i class='fa fa-plus'></i> Add More</button></td>" + "</tr>";
    ...
    function loadInitialGrid() {
        $.ajax({
            url: baseURL() + 'getItemSuggestionList', type: 'GET', success: function (res) {
